...
class PasswordEncoderFactoryTest_Encryption_Argon2Id: StringSpec({
    ...
    "argon2id(prefix): 팩토리에서 생성된 argon2id 인코더의 encode 메서드는 {argon2}로 시작하는 문자열을 생성한다." {
    ...
    "argon2id(parameters): Argon2id 인코더로 인코딩 된 문자열에 입력한 파라미터가 올바른 양식으로 입력된다." {
        val encoder = factory.create(argon2IdOption)
        val rawPassword = "abcd1234"

        val encodedPassword = encoder.encode(rawPassword)

        assertTrue { (encodedPassword.startsWith("$argon2Prefix\$argon2id\$v=19\$m=49343,t=1,p=1")) }
    }

    "argon2id(encode, matches): 암호화한 후 동일한 평문 비밀번호와 비교하면 일치한다." {
        val encoder = factory.create(argon2IdOption)
        val rawPassword = "abcd1234"
    ...
    "argon2id(encode, matches): 암호화한 후 다른 평문 비밀번호와 비교하면 일치하지 않는다." {
        val encoder = factory.create(argon2IdOption)
        val rawPassword = "abcd1234"
